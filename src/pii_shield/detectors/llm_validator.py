...
2. Send SENTENCES containing low-confidence matches (not full text)
3. Group matches by sentence for efficient API batching
"""
...
class LLMValidator:
    ...
    def validate_low_confidence(
        ...
        # Validate each sentence group (1 API call per sentence)
        for sentence, sentence_start, match_list in sentence_groups:
            validations = self._validate_sentence(sentence, sentence_start, match_list)
